...
(function () {
  ...
  function navigate(path, push) {
    // loading.tw skeleton (docs/commands-reference.md): the boundary ships
    // hidden; show it while the navigation fetch runs, hide it again after.
    var lb = document.querySelector("[data-loading-boundary]");
    function hideSkeleton() {
      if (lb) try { lb.style.display = "none"; } catch (e) { /* gone */ }
    }
    if (lb) {
      // Show via the framework's own display style (empty string would
      // fall back to the default block and fight the inline display:none).
      try { lb.style.display = "block"; } catch (e) { /* ignore */ }
    }
    fetch(path, {
      headers: { "X-TW-Navigate": "1" },
      // Hover-prefetch warms the HTTP cache WITHOUT the navigate header;
      // a real navigation must bypass that cache so the server can render
      // `(.)page.tw` interceptors (and other nav-specific output).
      cache: "reload",
    })
      ...
      .catch(function () { hideSkeleton(); window.location.href = path; });
  }
